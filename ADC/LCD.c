...
{
	clr_bit(PORTB,LCD_EN);   	// disable
	PORTC=cpy_u8Command;		//send commands
	clr_bit(PORTB,LCD_RS);		// RS = 0 --> instruction register
	clr_bit(PORTB,LCD_RW);		// RW = 0 --> write

				/* Enable pulse */
	set_bit(PORTB,LCD_EN);      // Enable
	_delay_us(1);
	clr_bit(PORTB,LCD_EN);   	// disable
	_delay_us(1);
}
...
void LCD_voidWriteData(u8 cpy_u8Data)
{
	...
	set_bit(PORTB,LCD_RS);		// RS = 1 --> data register
	clr_bit(PORTB,LCD_RW);		// RW = 0 --> write
